    ...
    },
    {
        ...
        "inputSchema": {
            ...
            "properties": {
                ...
                "resolved_only": {
                    "type": "boolean",
                    "description": "Only show resolved calls (default: false)",
                },
                "source": {
                    "type": "string",
                    "enum": ["scip", "heuristic", "any"],
                    "description": "Filter by resolution source: scip (compiler-verified, high confidence), heuristic (name-based, lower confidence), any (default)",
                },
                "max_depth": {
                    "type": "integer",
...
class ArchGraphMCP:
    ...
    def _handle_search_calls(self, arguments: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        source_filter = arguments.get("source", "any")
        max_depth = arguments.get("max_depth", 1)
        limit = arguments.get("limit", 20)

        # Build source filter clause
        source_clause = ""
        if source_filter == "scip":
...
async def run_mcp_server(**kwargs: Any) -> None:
    ...
    async def call_tool(name: str, arguments: dict[str, Any]) -> list[types.TextContent]:
        if name == "extract":
            # Run synchronously — blocks event loop but avoids
            # asyncio.to_thread deadlock with MCP stdio transport.
            result = arch._handle_extract_sync(arguments, None)
